# McAelan Remigio - Data Science Portfolio

## 📌 About Me
Hello! My name is McAelan Remigio, I am currently a 2nd year dual major in Statistics and Data Science B.S. I specialize in **data science, statistics, and leadership.**

## 🎓 Education
- **San Diego State University (SDSU)**  
  *B.S. in Data Science & Statistics*  
- Interested in **AI, Big Data, Finance, and Business Administration**

## 💼 Professional Experience
- **CORVD Computer, Information, and Data Sciences ORISE Fellow NCIRD Public Health** | **Centers for Disease Control and Prevention**  
  - Conducted a 4-year global WHO data analysis on Coronaviruses, improved reporting accuracy to 100%, and optimized data standardization codebase, reducing processing time by 40%
  - *Youngest undergraduate (1st year) in CDC cohort with master's and PhD researchers*
  - Codebase is not available due to privacy reasons

- **Lead Data Scientist | Data Science Initiative Student Consulting Group**  
  - Created a student-run data science consulting program leading **multiple teams (16 interns)** solving real-world problems
  
- **Data Scientist** | **SDSU Research Foundation**  
  - Conduct data science research for the **Mathematics Persistence through Inquiry and Equity (MPIE) Project**, analyzing **10,000+ HSI student data** using **R**
  

## 🚀 Leadership & Initiatives
- **Founder + Executive President** | **Data Science Initiative (DSI) at SDSU**  
  - Built a **100+ member** community, developed a data science curriculum, and partnered with organizations for job opportunities via directors and CEOs
  
- **Co-Founder + Data Analyst** | **GrowLuv SDSU**  
  - Leading PR and data analytics for **human abuse awareness initiatives**.

## 📊 Key Projects
- **WHO COVID Global Data Analysis & SARS-CoV-2 Sequencing Strategies**  
  - Collaborated with WHO experts like Laura Hughes Baker & Peter Cook to optimize pandemic response strategies.

## 🔧 Technical Skills
- **Programming:** Python, R, SQL
- **Data Science:** Machine Learning, Predictive Modeling, Data Visualization
- **Finance & Risk:** Investment Risk Analysis, Big Data in Finance
- **Tools:** Tableau, Power BI, Git

## 📬 Contact & Links
- **GitHub:** [Github]([https://github.com/your-profile](https://github.com/McAelanRemigio))
- **LinkedIn:** [LinkedIn]([https://www.linkedin.com/in/your-profile](https://www.linkedin.com/in/mcaelan/])_
